...
# --- 1. Configuration ---
# The plausible population file generated by evaluate_population_plausibility.py
PLAUSIBLE_POPULATION_FILE = "populations/evaluated/plausible_population.json"

# The original, full population file (needed to get the baseline grid)
ORIGINAL_POPULATION_FILE = "populations/population_crit0_pop100000.json"

# Directory where the output images will be saved
OUTPUT_DIR = "visualizations_plausible"

# The size of the highlight box to draw around the agent's critical position
HIGHLIGHT_BOX_SIZE = 0

# --- 2. Style Configuration (Copied from your original script) ---
...
# --- 3. Main Visualization Function (Modified for new inputs) ---
def plot_and_save_comparison(original_individual, modified_individual, output_path):
    """
    Generates and saves a side-by-side plot of the original grid vs. a plausible one.
    """
    # --- Extract Data ---
    ...
    plausibility_score = modified_individual.get("plausibility_score", "N/A")
    ...
    fig.suptitle(f"Plausible Counterfactual Comparison (Source Episode: {episode_id})", fontsize=18, weight='bold')
    ...
    title_text = f"Plausible Environment (Score: {plausibility_score:.3f})"
...
if __name__ == "__main__":
    # --- File Validation ---
    if not os.path.exists(PLAUSIBLE_POPULATION_FILE):
        print(f"❌ Error: Plausible population file not found at '{PLAUSIBLE_POPULATION_FILE}'")
        print("Please run evaluate_population_plausibility.py first.")
        ...
        # --- Load Data ---
        print(f"Loading plausible individuals from: {PLAUSIBLE_POPULATION_FILE}")
        with open(PLAUSIBLE_POPULATION_FILE, 'r') as f:
        ...
        original_individual = original_pop_data[0] # The baseline is always the first one
        ...
        if plausible_data:
            ...
            num_individuals = len(plausible_data)
            for i, plausible_individual in enumerate(plausible_data):
                filename = f"plausible_comparison_{i:03d}.png"
                output_path = os.path.join(OUTPUT_DIR, filename)

                print(f"[{i + 1}/{num_individuals}] Generating visualization -> {output_path}")
                plot_and_save_comparison(original_individual, plausible_individual, output_path)

            print("\n✅ Batch visualization of plausible chromosomes complete!")
        else:
            print("\nThe plausible population file is empty. No visualizations to generate.")